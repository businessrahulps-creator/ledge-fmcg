...
const healthLabels: Record<PerformanceHealth, string> = { high: "High Performer", medium: "Moderate", low: "Needs Attention" };
const healthColors: Record<PerformanceHealth, string> = { high: "#16a34a", medium: "#d97706", low: "#dc2626" };
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={{ flexDirection: "row", marginBottom: 14, gap: 16 }}>
          <View style={{ flex: 1, backgroundColor: "#f8fafc", padding: 10, borderRadius: 4, border: "1pt solid #e2e8f0" }}>
            <Text style={{ fontSize: 8, color: "#64748b", marginBottom: 4 }}>SALESPERSON DETAILS</Text>
            <Text style={{ fontSize: 10, fontFamily: "Helvetica-Bold", marginBottom: 2 }}>{salesperson.name}</Text>
            {salesperson.region ? <Text style={{ fontSize: 9, color: "#475569" }}>Region: {salesperson.region}</Text> : null}
            {salesperson.phone ? <Text style={{ fontSize: 9, color: "#475569" }}>Phone: {salesperson.phone}</Text> : null}
            {salesperson.email ? <Text style={{ fontSize: 9, color: "#475569" }}>Email: {salesperson.email}</Text> : null}
          </View>
          <View style={{ flex: 1, backgroundColor: "#f8fafc", padding: 10, borderRadius: 4, border: "1pt solid #e2e8f0" }}>
            <Text style={{ fontSize: 8, color: "#64748b", marginBottom: 4 }}>SUMMARY</Text>
            <Text style={{ fontSize: 10, fontFamily: "Helvetica-Bold", marginBottom: 2 }}>
              Total Revenue: {fmt(scorecard.totalRevenue)}
            </Text>
            <Text style={{ fontSize: 9, color: "#475569" }}>
              {orders.length} total orders · {scorecard.orderFrequency} orders/week
            </Text>
          </View>
        </View>
        ...
        <View style={{ border: "1pt solid #e2e8f0", borderRadius: 4, marginBottom: 14 }}>
          ...
          <View style={{ flexDirection: "row", padding: 6, backgroundColor: "#ffffff" }}>
            <Text style={{ flex: 1, fontSize: 9, fontFamily: "Helvetica-Bold", color: "#334155" }}>Performance Health</Text>
            <Text style={{ flex: 1, fontSize: 9, fontFamily: "Helvetica-Bold", color: healthColors[health], textAlign: "right" }}>
              {healthLabels[health]}
            </Text>
          </View>
        </View>
        ...
        {orders.length > 0 ? (
          <View style={{ border: "1pt solid #e2e8f0", borderRadius: 4 }}>
            <View style={{ flexDirection: "row", backgroundColor: "#f1f5f9", padding: 6, borderBottom: "1pt solid #e2e8f0" }}>
              <Text style={{ width: "15%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569" }}>Order #</Text>
              <Text style={{ width: "15%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569" }}>Date</Text>
              <Text style={{ width: "25%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569" }}>Dealer</Text>
              <Text style={{ width: "15%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569", textAlign: "right" }}>Amount</Text>
              <Text style={{ width: "15%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569", textAlign: "right" }}>Savings</Text>
              <Text style={{ width: "15%", fontSize: 8, fontFamily: "Helvetica-Bold", color: "#475569", textAlign: "right" }}>Status</Text>
            </View>
            {orders.slice(0, 30).map((o, i) => (
              <View key={i} style={{ flexDirection: "row", padding: 6, borderBottom: "0.5pt solid #e2e8f0", backgroundColor: i % 2 === 0 ? "#ffffff" : "#fafafa" }}>
                <Text style={{ width: "15%", fontSize: 8, color: "#1e293b" }}>{o.orderNumber}</Text>
                <Text style={{ width: "15%", fontSize: 8, color: "#475569" }}>
                  {new Date(o.date + "T00:00:00").toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}
                </Text>
                <Text style={{ width: "25%", fontSize: 8, color: "#475569" }}>{o.distributorName}</Text>
                <Text style={{ width: "15%", fontSize: 8, color: "#1e293b", textAlign: "right" }}>{fmt(o.total)}</Text>
                <Text style={{ width: "15%", fontSize: 8, color: o.schemeSavings > 0 ? "#16a34a" : "#94a3b8", textAlign: "right" }}>
                  {o.schemeSavings > 0 ? `-${fmt(o.schemeSavings)}` : "—"}
                </Text>
                <Text style={{ width: "15%", fontSize: 8, textAlign: "right", color: o.paymentStatus === "paid" ? "#16a34a" : o.paymentStatus === "partial" ? "#d97706" : "#dc2626" }}>
