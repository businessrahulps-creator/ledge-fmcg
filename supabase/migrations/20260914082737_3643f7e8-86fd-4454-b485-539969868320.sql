...
BEGIN
  ...
  -- Goods already left (dispatched/delivered) but never billed: bill only, leave stock alone.
  ...
  v_date := COALESCE(p_dispatch_date, CASE WHEN v_already_out THEN v_order.dispatch_date END, CURRENT_DATE);
