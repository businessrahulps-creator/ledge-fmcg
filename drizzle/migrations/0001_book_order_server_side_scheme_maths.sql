...
BEGIN
  ...
  -- Recalculate scheme savings here; whatever the phone sent is ignored.
  FOR v_s IN
  ...
  LOOP
    ...
    IF v_s.is_combinable THEN
      ...
      VALUES ('00000000-0000-0000-0000-000000000000'::uuid, v_s.id, v_s.name, v_label, v_savings);
      DELETE FROM order_schemes WHERE order_id = '00000000-0000-0000-0000-000000000000'::uuid;
      -- placeholder row removed; real rows are written after the order exists
  ...
  SELECT v_order_id, t.product_id, COALESCE(p.name, ''), t.quantity, t.unit_price, t.amount, t.quantity, t.amount
    FROM tmp_book_lines t LEFT JOIN products p ON p.id = t.product_id;

  -- Record the schemes that actually applied (recomputed, not what the phone sent)
  IF v_best_id IS NOT NULL THEN
    INSERT INTO order_schemes (order_id, scheme_id, scheme_name, scheme_label, savings)
    VALUES (v_order_id, v_best_id, v_best_name, v_best_label, v_best_savings);
  END IF;

  FOR v_s IN
    SELECT * FROM schemes s
     WHERE s.company_id = v_company AND s.is_active AND s.is_combinable
       AND s.valid_from <= p_date
       AND (s.valid_until IS NULL OR s.valid_until >= p_date)
       AND (s.dealer_id IS NULL OR s.dealer_id = p_distributor_id)
  LOOP
    ...
    INSERT INTO order_schemes (order_id, scheme_id, scheme_name, scheme_label, savings)
    VALUES (v_order_id, v_s.id, v_s.name, v_label, v_savings);
  END LOOP;
